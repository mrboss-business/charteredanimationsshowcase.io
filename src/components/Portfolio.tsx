...
const Portfolio = () => {
  ...
  return (
    <section id="portfolio" className="py-section bg-shaded">
      <div className="max-w-container mx-auto px-4">
        {/* Section Header */}
        <div className="text-center mb-12 fade-up">
          <h2 className="font-heading font-bold text-4xl lg:text-5xl text-milk mb-4">
            Our Work
          </h2>
          <p className="text-milk/70 font-body text-lg max-w-2xl mx-auto">
            Explore our portfolio of creative projects across various industries
          </p>
        </div>

        {/* Filter Buttons */}
        <div className="flex flex-wrap justify-center gap-4 mb-12 fade-up-delay">
          {filters.map((filter) => (
            <button
              key={filter}
              ...
              className={`px-6 py-2 rounded-full font-body font-medium transition-all duration-300 ${
                activeFilter === filter
                  ? "bg-milk text-coffee"
                  : "bg-milk/10 text-milk hover:bg-milk/20"
              }`}
            >
              {filter}
            </button>
