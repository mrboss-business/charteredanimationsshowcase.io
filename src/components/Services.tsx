...
const Services = () => {
  ...
  return (
    <section id="services" className="py-section bg-milk">
      <div className="max-w-container mx-auto px-4">
        {/* Section Header */}
        <div className="text-center mb-16 fade-up">
          <h2 className="font-heading font-bold text-4xl lg:text-5xl text-coffee mb-4">
            What We Do
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 fade-up-delay">
          {services.map((service, index) => (
            <div
              key={index}
              className="card-elegant bg-white p-6 relative group"
            >
              {/* Popular Badge */}
              {service.popular && (
                <div className="absolute -top-3 -right-3 bg-coffee text-milk text-xs font-semibold px-3 py-1 rounded-full">
                  Popular
                </div>
              )}
