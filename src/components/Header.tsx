  ...
  return (
    ...
    >
      <div className="max-w-container mx-auto px-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-milk/90 hover:text-milk font-body font-medium transition-colors duration-200"
            >
              {link.label}
            </a>
          ))}
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden text-milk p-2"
      ...
      {/* Mobile Navigation */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-shaded border-t border-milk/20">
          <nav className="px-4 py-6 space-y-4">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="block text-milk/90 hover:text-milk font-body font-medium py-2"
